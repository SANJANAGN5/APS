...
```{r}
simTrial = function(){
  Data=sample(s,size = 10,replace = T,prob = p)
  return(Data)
}
print(simTrial())
```
...
```{r}
print(mean(apply(simulatedData, 2, checkEvent)))#probabilty of getting head for 3 tosses 
```
...
```{r}
simTrial = function(){
  return(sample(s, size = 5,replace = T, prob = P))
}
print(simTrial())
```
...
```{r}
print(apply(simulatedData, 2, checkEvent))
mean(apply(simulatedData, 2, checkEvent))#probabilty of S1 compilation
```
